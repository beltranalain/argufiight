...
// GET /api/debates/[id]/check-verdicts-public - Public endpoint to check verdict status (no auth required)
export async function GET(
  ...
  try {
    ...
    if (!deepseekConfigured) {
      blockers.push(`DeepSeek API key not configured: ${deepseekError || 'Unknown error'}`)
    }
    ...
    return NextResponse.json({
      ...
      debate: {
        ...
        hasVerdicts: debate.verdicts.length > 0,
      },
      verdictGeneration: {
