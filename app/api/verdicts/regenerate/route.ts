...
async function generateAppealApprovalReason(
  ...
  // Collect reasoning from all new verdicts
...
// POST /api/verdicts/regenerate - Regenerate verdict for an appealed debate
export async function POST(request: NextRequest) {
  try {
    ...
    if (debate.status !== 'APPEALED') {
      return NextResponse.json(
        { error: 'Debate is not in appealed status' },
        { status: 400 }
      )
    }
    ...
    if (newVerdicts.length === 0) {
      ...
      return NextResponse.json(
        { error: 'Failed to generate new verdicts' },
        { status: 500 }
      )
    }
    ...
    if (verdictFlipped && finalWinnerId) {
      ...
      // originalWinnerId stores the winner BEFORE the appeal
      // If it's null, the original verdict was a tie
      const originalWinnerId = debate.originalWinnerId
      ...
      if (originalWinnerId === debate.challengerId) {
        // Original winner was challenger, reverse their win
        ...
        // Original winner was opponent, reverse their win
        if (debate.opponentId) {
          await prisma.user.update({
            where: { id: debate.opponentId },
            data: {
        ...
        // Original was a tie (originalWinnerId is null), reverse the tie stats
        await prisma.user.update({
          where: { id: debate.challengerId },
          data: {
    ...
    console.error('Regenerate verdict error:', error)
    return NextResponse.json(
