...
export function PublicHomepage() {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-electric-blue text-xl">Loading...</div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-purple-950 via-purple-900 to-indigo-950 relative overflow-hidden">
      ...
      {/* Footer */}
      <FooterSection section={sortedSections.find(s => s.key === 'footer' && s.isVisible)} />
    </div>
  )
}
...
function HomepageSectionComponent({ section, index }: { section: HomepageSection; index: number }) {
  ...
  // Index 0 = right image, Index 1 = left image, Index 2 = right image
  const isImageRight = index % 2 === 0
  const primaryImage = sectionImages[0] // Use first image as primary

  return (
    <section className="py-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        {/* Glassmorphism card - consistent styling */}
...
function AppDownloadSection({ section }: { section: HomepageSection }) {
  // Get app store buttons - first two buttons are for App Store and Google Play
  ...
  // Get app store logos from images - check alt text first, then order
  // Sort images by order to ensure consistent matching
  const sortedImages = [...section.images].sort((a, b) => a.order - b.order)
  
  // Find App Store image - check alt text first, then fall back to order
  ...
  // Find Google Play image - check alt text first, then fall back to order
  // If we found App Store at order 0, Google Play should be at order 1
  // Otherwise, use the first image that's not the App Store image
  const googlePlayImage = sortedImages.find(img => {
    if (img.id === appStoreImage?.id) return false // Don't use the same image
    const altLower = img.alt?.toLowerCase() || ''
    return altLower.includes('google play') || 
           altLower.includes('google') ||
           altLower.includes('play store') ||
           (appStoreImage?.order === 0 && img.order === 1) || // If App Store is at 0, Google Play is at 1
           (!appStoreImage && img.order === 0) || // If no App Store, first image could be Google Play
           (appStoreImage && img.order === 1) // If App Store exists, second image is Google Play
  })

  // Show images even if buttons don't exist (images are clickable)
          ...
          {(hasAppStore || hasGooglePlay) && (
            <div className="flex flex-col sm:flex-row gap-6 justify-center items-center">
              {hasAppStore && (
                <>
                  {appStoreImage ? (
                    // Use image linkUrl if available, otherwise button URL, otherwise don't make clickable
...
function FooterSection({ section }: { section: HomepageSection | undefined }) {
  return (
