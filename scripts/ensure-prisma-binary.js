...
console.log('🔍 Checking for Prisma RHEL binary...')

if (!existsSync(prismaClientDir)) {
  console.error('❌ Prisma client directory not found:', prismaClientDir)
  console.error('   Run: npx prisma generate')
  process.exit(1)
}

if (existsSync(binaryPath)) {
  console.log('✅ Prisma RHEL binary found:', binaryPath)
  ...
  if (existsSync(join(rootDir, '.next'))) {
    try {
      ...
      console.log('✅ Copied binary to .next directory:', nextBinaryPath)
    } catch (error) {
      console.warn('⚠️  Could not copy binary to .next (may not exist yet):', error.message)
    }
  }
} else {
  console.error('❌ Prisma RHEL binary NOT found:', binaryPath)
