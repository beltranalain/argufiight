/**
 * Complete End-to-End Test: King of the Hill Tournament with 5 Users
 * Tests the full tournament flow from creation to completion
 * Verifies simultaneous submissions work correctly
 */
...
async function createTestUsers() {
  ...
  for (let i = 1; i <= 5; i++) {
    const email = `koth-5-test-${i}-${baseTimestamp}@test.com`
    const username = `koth-5-test-${i}-${baseTimestamp}`
...
async function createTournament(users: any[]) {
  ...
  const tournament = await prisma.tournament.create({
    data: {
      name: `Test KOTH 5-User Full Test - ${Date.now()}`,
      description: 'Complete end-to-end test tournament with 5 participants',
...
async function submitStatementsForAllParticipants(debateId: string, users: any[], roundNumber: number) {
  ...
  if (participants.length === 0) {
    throw new Error(`No active participants found for debate ${debateId}`)
  }
  ...
  if ((debate?.statements.length || 0) < participants.length) {
    throw new Error(`Not all participants submitted! Expected ${participants.length}, got ${debate?.statements.length || 0}`)
  }
...
async function processRound(debateId: string, roundNumber: number) {
  ...
  if (isKingOfTheHill && debate.challengeType === 'GROUP') {
    ...
    // For finals (1v1), mark debate as COMPLETED first, then generate verdicts
    console.log(`  🔧 Marking finals debate as COMPLETED...`)
...
async function runTest() {
  try {
    console.log('🧪 Starting King of the Hill 5-User Complete End-to-End Test\n')
